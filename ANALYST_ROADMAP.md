...
> **Status: M5 DONE-as-negative-result (2026-06-30) + M4 DONE (2026-07-01) — see
> `backend/app/ml/FINDINGS.md` for both.** M5 (LightGBM screen-ranker, `app/ml/`): no stable OOS
> edge over the hand-screen; the flattering quarterly +0.0168 was retraining variance (proved by a
> frozen-model isolation experiment); shelved as a serving model, hand-screen stays. M4 (point-in-time
> panel): **(1) survivorship fix** — `app/universe/history.py` + committed `membership_history.json`
> (free fja05680/sp500 interval history; `constituents_asof()`); 727 names were members 2016–2026 vs
> the 503 survivors the backtest saw; 98/224 removed names recovered with full free price history
> (universe with data 503→617; the ~126 acquired/delisted rest remain the un-fillable residual);
> per-(name,date) gate in `evaluate.py`/`ml/panel.py` (also stops recent joiners appearing pre-
> membership). **(2) Exact filed-date gating** — `financials.filed_date` (migration `d7a2c4e8b910`)
> = EARLIEST SEC `filed` per quarter (deliberately min, not `_pick`'s restatement-max); 99.6%
> backfilled; median real lag 35d vs the blanket 75d. **(3) Materialized versioned panel** —
> `panel_versions`/`panel_rows` (migration `e9b3d5f1c820`), `python -m app.ml.materialize`,
> `python -m app.ml.run --panel-version N` pins exact training rows. **Headline re-measurement:
> the hand-screen baseline +0.0174 was ~HALF survivorship bias** — same dates/gating, PIT universe:
> +0.0090 (t 0.5); filed-date gating alone moved nothing (+0.0170). On PIT panels both GBM and
> hand-screen are ~zero OOS (2020+). Every future edge claim must cite a pinned PIT panel version.
|---|--------|--------|--------|-----------|
...
| M4 | **Panel assembly** — the point-in-time feature/label store (THE prerequisite; the real long-pole) — ✅ **DONE 2026-07-01** (survivorship fix + exact filed-date gating + materialized versioned panel; see the 2026-07-01 Status entry + `app/ml/FINDINGS.md`) | n/a — infrastructure | EDGAR `companyfacts` filed-date-gated fundamentals (have) + ~10y prices (extend lookback) + consensus snapshots (accrue from 4.1; early panel simply lacks revisions features) + **historical index constituents** (free GitHub lists — else survivorship bias) | build incrementally from 4.1/6.1 |
